...
describe('Category Theory', () => {
  it('proof composition definition law', () => {
    // h = g . f 
    const g_after_f = isGreaterThan5(findLength('tiny'))
    const h = flow(findLength, isGreaterThan5)('tiny')
    expect(h).toEqual(g_after_f)
  })

  it('proof composition association', () => {
    // ( h . g ) . f 
    const h_after_g_after_f = flow(findLength, flow(isGreaterThan5, convertToShortOrLong))
    // h . ( g . f )
    const h_after_g_and_f = flow(flow(findLength, isGreaterThan5), convertToShortOrLong)

    // ( h . g ) . f = h . ( g . f )
    expect(
      h_after_g_after_f('tiny')
    ).toEqual(
